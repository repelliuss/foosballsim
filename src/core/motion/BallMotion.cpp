...
BallMotion::BallMotion() noexcept : ravg{constants::default_run_avg_cap} {}
...
void BallMotion::_ready() {
  ...
  ravg.add(transform.origin.x, transform.origin.z);
}

void BallMotion::on_new_position(int x, int z) {
  ravg.add(Transformation::transform_x(x), Transformation::transform_z(z));
}

void BallMotion::_process(float deltatime) {
  if (ravg.next_pos(transform.origin.x, transform.origin.z, deltatime)) {
